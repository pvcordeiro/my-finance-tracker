...
interface EntryFormProps {
  ...
  onAddEntry: () => void
  onUpdateEntry: (id: string, field: string, value: string | number, monthIndex?: number) => void
...
export function EntryForm({
  ...
  return (
    <Card className={cn("transition-all duration-200", type === "income" ? "income-card" : "expense-card")}>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm sm:text-base font-normal text-muted-foreground">
                  €{entries.reduce((sum, entry) => sum + calculateTotal(entry.amounts), 0).toFixed(2)}
                </span>
                {isOpen ? (
          ...
          <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 sm:gap-4">
                        {rollingMonths.map((month, index) => (
                          <div key={month} className="space-y-2">
                            <label className="text-xs font-medium text-muted-foreground block">{month}</label>
                            <div className="relative">
                              ...
                              <Input
                                ...
                                onChange={(e) =>
                                  onUpdateEntry(entry.id, "amount", Number.parseFloat(e.target.value) || 0, index)
                                }
            ...
            <Button
              ...
              className={cn(
                ...
                type === "income" ? "border-emerald-200 hover:bg-emerald-50" : "border-red-200 hover:bg-red-50",
              )}
            >
