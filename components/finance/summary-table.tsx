...
export function SummaryTable({ data }: SummaryTableProps) {
  ...
  const monthlyData = months.map((month, index) => {
    ...
    // Add bank amount to current month
    let bankAmount = 0
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-xs sm:text-sm font-medium text-emerald-700">Total Income</p>
                <p className="text-lg sm:text-2xl font-bold text-emerald-800 truncate">€{annualIncome.toFixed(2)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                ...
                <p className="text-lg sm:text-2xl font-bold text-red-800 truncate">€{annualExpenses.toFixed(2)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-r from-primary/10 to-accent/10 border-primary/20">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-xs sm:text-sm font-medium text-primary">Net Savings</p>
                <p
                  className={cn(
        ...
        <Card className="bg-gradient-to-r from-accent/10 to-primary/10 border-accent/20">
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-xs sm:text-sm font-medium text-accent">Final Balance</p>
                <p
                  className={cn(
                    "text-lg sm:text-2xl font-bold truncate",
            ...
            <table className="w-full border-collapse min-w-[500px]">
              <thead>
                <tr className="border-b bg-muted/50">
                  <th className="text-left p-3 sm:p-3 font-semibold text-sm sm:text-base">Month</th>
                  <th className="text-right p-3 sm:p-3 font-semibold text-emerald-700 text-sm sm:text-base">Income</th>
                  <th className="text-right p-3 sm:p-3 font-semibold text-red-700 text-sm sm:text-base">Expenses</th>
                  <th className="text-right p-3 sm:p-3 font-semibold text-sm sm:text-base">Balance</th>
                </tr>
              </thead>
              <tbody>
                {monthlyData.map((row, index) => (
                  ...
                  >
                    <td className="p-3 sm:p-3 font-medium">
                      <div className="flex items-center gap-2">
                        <span className="text-sm sm:text-base">{row.month}</span>
                        {row.isCurrentMonth && (
