          ...
          <div className="space-y-4">
            {chartData.map((item, index) => (
              <div key={index} className="space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  <span>{item.month}</span>
                  <span className={item.net >= 0 ? "text-emerald-600" : "text-red-600"}>€{item.net.toFixed(0)}</span>
                </div>
                <div className="relative h-6 bg-gray-100 rounded">
                  ...
                  <div
                    ...
                    style={{
                      left: `${maxNet > minNet ? ((0 - minNet) / (maxNet - minNet)) * 100 : 50}%`,
                    }}
                  />
                  {/* Net balance bar */}
                  <div
                    className={`absolute top-1 bottom-1 rounded ${item.net >= 0 ? "bg-emerald-500" : "bg-red-500"}`}
                    style={{
                      left:
                        item.net >= 0
                          ? `${maxNet > minNet ? ((0 - minNet) / (maxNet - minNet)) * 100 : 50}%`
                          : `${maxNet > minNet ? ((item.net - minNet) / (maxNet - minNet)) * 100 : 45}%`,
                      width: `${maxNet > minNet ? (Math.abs(item.net) / (maxNet - minNet)) * 100 : 10}%`,
                    }}
