...
export async function createSession(userId) {
  ...
  // Get user's preferred group (last selected if still accessible) or first group
...
export async function switchGroup(sessionToken, groupId) {
  ...
  // Update the current group in session and user's preference
  await new Promise((resolve, reject) => {
    db.run(
      `UPDATE sessions SET current_group_id = ? WHERE token = ?`,
  ...
  // Also update the user's last selected group
  await new Promise((resolve, reject) => {
    db.run(
