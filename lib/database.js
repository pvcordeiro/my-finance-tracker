    ...
    return new Promise((resolve, reject) => {
      ...
      db = new sqlite3.Database(dbPath, (err) => {
        ...
        db.exec(initSQL, (err) => {
            ...
            const ensureSettingsAndCleanup = () => {
              db.run(
                ...
                (err) => {
                  ...
                  db.run(
                    `INSERT OR IGNORE INTO settings (key, value) VALUES ('privacy_mode', 'false')`,
                    (err) => {
                      if (err) {
                        console.error(
                          "Error inserting privacy_mode setting:",
                          err
                        );
                      }
                    }
          ...
          }); // Close runMigrations().then()
        }); // Close db.exec()
      }); // Close new sqlite3.Database()
    }); // Close new Promise()
  }
  return db;
}
