          ...
          db.all("PRAGMA table_info(users)", [], (err, rows) => {
            ...
            if (!columnNames.includes("language")) {
              db.run(
                ...
                (err) => {
                  if (err) console.error("Error adding language column:", err);
                  else console.log("Added language column to users table");
                }
              );
            }
            ...
            if (!columnNames.includes("currency")) {
              db.run(
                ...
                (err) => {
                  if (err) console.error("Error adding currency column:", err);
                  else console.log("Added currency column to users table");
                }
              );
            }
          });
              ...
              db.run(
                ...
                (err) => {
                  ...
                  db.run(
                    ...
                    [process.env.ENABLE_BALANCE_HISTORY || "true"],
                    (err) => {
                      if (err) {
                        console.error(
                ...
                (err, existingAdmin) => {
                  ...
                  db.run(
                    ...
                    function (err) {
                      if (err) {
                        console.error(
                          "Error creating initial admin user:",
                          err
                        );
                        ensureSettingsAndCleanup();
                        return;
