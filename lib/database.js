    ...
    return new Promise((resolve, reject) => {
      ...
      db = new sqlite3.Database(dbPath, (err) => {
        ...
        db.exec(initSQL, (err) => {
          ...
          // Add user_id columns if they don't exist (ignore errors if they already exist)
          db.run(
            ...
            db.run(
              ...
              function (err) {
                if (err) {
                  ...
                  if (adminUserId) {
                    // Create admin's personal group
                    const adminGroupName = "Admin Group";
                    db.run(
                      `INSERT OR IGNORE INTO groups (name, created_by) VALUES (?, ?)`,
